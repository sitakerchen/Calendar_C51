...
* ÎÄ¼ş£º Calendar.h                                                     *
* ×÷Õß£º cxl Hunau                                                  *
* ÓÊÏä£º sitakerchen@gmail.com                                       *
*                                                                  *
*¡¾°æÈ¨¡¿Copyright(C)cxl  All Rights Reserved  *
*¡¾ÉùÃ÷¡¿´Ë³ÌĞò½öÓÃÓÚÑ§Ï°Óë²Î¿¼£¬ÒıÓÃÇë×¢Ã÷°æÈ¨ºÍ×÷ÕßĞÅÏ¢£¡        *
*                                                                  *
********************************************************************/
#ifndef  __Calendar_h__                //Ìõ¼ş»ã±àÖ¸Áî
#define  __Calendar_h__                //±ÜÃâÖØ¸´µ÷ÓÃ
...
void calendar_showTime(SYSTEMTIME *Time)
{
	...
	while (key != EXIT)	//	ÓÉÓÚ²ÉÓÃµÄÊÇ³ÌĞòÊµÏÖ°´¼üÉ¨Ãè¶ø·ÇÖĞ¶Ï£¬ËùÒÔÉ¨Ãè»áÓĞÒ»¸ö´°¿ÚÆÚ£¬´°¿ÚÆÚÖ®ÍâÎŞ·¨ÓĞĞ§²ÉÑù£¬¹ÊÍÆ¼öÊ¹ÓÃ³¤°´ÍË³öµÄ·½·¨¡£
	{
		key = read_key();
		DS1302_GetTime(Time);
		TimeToStr(Time);
...
uchar calendar_month_Word2value(uchar str[4])
{
	...
	else return 0; 	//	ÀÁµÃĞ´ÁË£¬¿ÉÒÔ×ÔĞĞÌí¼Ó		
...
void calendar_editAndLookupLunarDate(SYSTEMTIME* Time)
{
	...
	while (key != EXIT)
	{
		...
		if (key == 1)
		{
			...
			if (slnum == 1)
			{
//				DS1302_Stop(1);	//	ÔİÍ£¼ÆÊ±
				lcd_wcmd(FLine + 10);	// ÈÕ Î»ÖÃ
				lcd_wcmd(0x0f);	//	·´°×±
			}
			if (slnum == 2)
			{
		...
		if (slnum != 0)
		{
			if (key == 2)
			{
					if(slnum == 1)
					{
						...
						if(d >= m2d[(int) m ] + 1 + tem) d=0;			//ÅĞ¶ÏÊÇ·ñ³¬³ö·¶Î§
						calendar_wdate(10, d);		//ÏÔÊ¾
						lcd_wcmd(FLine + 10);
					}
					if(slnum == 2)
					{
						m ++;					  //Ôö¼ÓÔÂ
						if(m >= 13) m = 0;				  //ÅĞ¶ÏÊÇ·ñ³¬³ö·¶Î§
						calendar_wdate(7, m);		  //ÏÔÊ¾
						lcd_wcmd(FLine + 7);
					}
					if(slnum == 3)
					{
						y ++;					  //Ôö¼ÓÄê
						calendar_wdate(4, y);		  //ÏÔÊ¾
						lcd_wcmd(FLine + 4);
					}	
			}
			if (key == 3)
			{
					...
					{
						d --;					//Ôö¼ÓÈÕ
						if(d <= -1 ) d = m2d[(int) m] + (m == 2)?Calendar_judge(y):0;			//ÅĞ¶ÏÊÇ·ñ³¬³ö·¶Î§
						calendar_wdate(10, d);		//ÏÔÊ¾
						lcd_wcmd(FLine + 10);
